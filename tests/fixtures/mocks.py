...
def minimized_test_delays() -> Generator[None]:
    ...
    """
    with (
        patch("kobold.worker.WORKER_ERROR_BACKOFF", 0.01),
        patch("kobold.watcher.WATCH_DEBOUNCE_MS", 1),
        patch.dict(
            "os.environ",
            {
                "KB_WATCH_POLL_DELAY_MS": "1",
                "KB_WORKER_POLL_INTERVAL": "0.01",
            },
        ),
        patch("kobold.scheduler.RECONCILE_INTERVAL_MINUTES", 1),
        patch("kobold.metadata.base.SCRAPER_MIN_DELAY", 0.0),
        patch("kobold.metadata.base.SCRAPER_JITTER_MAX", 0.0),
    ):
        yield
...
def mock_amazon_provider() -> Generator[AsyncMock]:
    with patch("kobold.metadata.manager.AmazonProvider") as MockProvider:
...
def mock_goodreads_provider() -> Generator[AsyncMock]:
    with patch("kobold.metadata.manager.GoodreadsProvider") as MockProvider:
        mock = MockProvider.return_value
        mock.fetch_metadata = AsyncMock(return_value=None)
        yield mock
...
def mock_kepub_converter() -> Generator[AsyncMock]:
    with patch("kobold.worker.KepubConverter") as MockConverterCls:
